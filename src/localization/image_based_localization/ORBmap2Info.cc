...
#define  bytes_per_descriptor 32  // For ORB Brief
...
int main (int argc, char **argv)
{
    if( argc != 3 )
    {
        ...
        std::cout << " -                                                                                                   - " << std::endl;
        std::cout << " -    ORBmap2Info - Extract camera and feature information from Bundler output files, save feature   - " << std::endl;
        ...
        std::cout << " -                                                                                                   - " << std::endl;
        std::cout << " - usage: ORBmap2Info orbslam_output  outfile                                              - " << std::endl;
        std::cout << " - Parameters:                                                                                       - " << std::endl;
        std::cout << " -   orbslam_output                                                                                  - " << std::endl;
        std::cout << " -     Filename of an output file (usually called map.bin) generated by orbslam.                      - " << std::endl;
        std::cout << " -                                                                                                   - " << std::endl;
        std::cout << " - outfile                                                                                          - " << std::endl;
        std::cout << " -     The output generated by ORBmap2Info. It is a binary file of the following structure:          - " << std::endl;
    ...
    for( uint32_t i=0; i<nb_cameras; ++i )
    {
        ...
        if((max_features_per_cam < nb_features_per_cam[i] )){
            max_features_per_cam  = nb_features_per_cam[i];
            max_id = i;
        }

        //std::cout<<nb_features_per_cam[i]<<std::endl;
    ...
    ////
    // save all data
    std::cout << "->saving the information from bundler to " << argv[2] << std::endl;

    std::ofstream ofs( argv[2], std::ios::out | std::ios::binary );

    if( !ofs.is_open() )
    {
        std::cerr << " Could not write information " << argv[2] << std::endl;
        return 1;
    }
    ...
    for( uint32_t i=0; i<nb_points; ++i )
    {
        ...
        for( uint32_t j=0; j<nb_cams_visible_in; ++j )
        {
            ...
            unsigned char *desc = new unsigned char[bytes_per_descriptor];
            for( uint32_t k=0; k<bytes_per_descriptor; ++k )
                desc[k] = feature_infos[i].descriptors[bytes_per_descriptor*j+k];
            ...
            ofs.write( (char*) desc, bytes_per_descriptor*sizeof( unsigned char ) );
